...
'''
This func should return the information of the . 
The information should include the description of the  and
also the the items in the  if any
'''
def look_around():
    #write code here
    pass
...
'''
Write a function to take an item which is present in the .
note: you can take only those items which are present in the .
...
3. Took the {item}
4. There is nothing to take in the .
'''
...
'''
Write a function to go to a different  based on the direction specified.
Call handel_rats() function to check if rats exist
Everytime user enters a new , give the description of the .
'''
def go_to(tokens):
...
The treasure box can be unlocked only if the user is in the bed and has "old_book" in the inventory
...
def play_game():
    ...
    while True:
        ...
        if current_ == "bed" and "treasure box" in inventory:
            print(s["bed"]["ending"])
